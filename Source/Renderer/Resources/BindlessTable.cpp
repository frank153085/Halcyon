...
[[nodiscard]] Halcyon::Core::Error makeError(Halcyon::Core::ErrorCode code,
                                             std::string message,
                                             std::string context = {}) {
    return Halcyon::Core::Error{code, std::move(message), std::move(context)};
}

[[nodiscard]] std::uint32_t capacityFor(const BindlessTableConfig& config,
                                        DescriptorType type) noexcept {
    switch (type) {
    case DescriptorType::SampledImage: return config.sampledImageCapacity;
    case DescriptorType::StorageImage: return config.storageImageCapacity;
    case DescriptorType::UniformBuffer: return config.uniformBufferCapacity;
    case DescriptorType::StorageBuffer: return config.storageBufferCapacity;
    case DescriptorType::Sampler: return config.samplerCapacity;
...
DescriptorSlotAllocator::DescriptorSlotAllocator(std::uint32_t capacity,
...
Halcyon::Result<void> DescriptorSlotAllocator::initialize(
    DescriptorSlotAllocatorConfig config) {
    ...
    if (!slots_.empty()) {
        return Halcyon::Result<void>::failure(
            makeError(Halcyon::Core::ErrorCode::AlreadyExists,
    ...
    try {
        ...
        return Halcyon::Result<void>::failure(
            makeError(Halcyon::Core::ErrorCode::OutOfMemory,
                      "failed to allocate descriptor slot table"));
    } catch (const std::exception& exception) {
        return Halcyon::Result<void>::failure(
            makeError(Halcyon::Core::ErrorCode::InvalidState, exception.what(),
...
Halcyon::Result<DescriptorHandle> DescriptorSlotAllocator::allocate() {
    ...
    if (freeSlots_.empty()) {
        return Halcyon::Result<DescriptorHandle>::failure(
            makeError(Halcyon::Core::ErrorCode::OutOfMemory,
                      "bindless descriptor table capacity exhausted"));
    }
    ...
    ++liveCount_;
    return Halcyon::Result<DescriptorHandle>::success(
        DescriptorHandle{index, slot.generation});
}
...
Halcyon::Result<void> DescriptorSlotAllocator::release(DescriptorHandle handle,
                                                       std::uint64_t retireTimelineValue) {
    ...
    try {
        ...
        return Halcyon::Result<void>::failure(
            makeError(Halcyon::Core::ErrorCode::OutOfMemory,
                      "failed to queue descriptor slot retirement"));
    } catch (const std::exception& exception) {
        return Halcyon::Result<void>::failure(
            makeError(Halcyon::Core::ErrorCode::InvalidState, exception.what(),
...
Halcyon::Result<void> DescriptorSlotAllocator::touch(DescriptorHandle handle,
...
std::size_t DescriptorSlotAllocator::collect(
    std::uint64_t completedTimelineValue,
...
Halcyon::Result<void> BindlessTable::initialize(BindlessTableConfig config) {
    if (initialized_) {
        return Halcyon::Result<void>::failure(
            makeError(Halcyon::Core::ErrorCode::AlreadyExists,
                      "bindless table is already initialized"));
    }
...
Halcyon::Result<DescriptorHandle> BindlessTable::allocate(DescriptorType type,
    ...
    if (!initialized_) {
        return Halcyon::Result<DescriptorHandle>::failure(
            makeError(Halcyon::Core::ErrorCode::InvalidState,
                      "bindless table is not initialized"));
    }
    completedTimeline_ = std::max(completedTimeline_, completedTimelineValue);
...
Halcyon::Result<void> BindlessTable::update(DescriptorType type,
    ...
    if (!allocators_[i].contains(handle)) {
        return Halcyon::Result<void>::failure(
            makeError(Halcyon::Core::ErrorCode::NotFound,
...
Halcyon::Result<void> BindlessTable::release(DescriptorType type,
                                             DescriptorHandle handle,
                                             std::uint64_t retireTimelineValue) {
    if (!validType(type)) {
        return Halcyon::Result<void>::failure(invalidTypeError());
...
Halcyon::Result<void> BindlessTable::touch(DescriptorType type,
                                           DescriptorHandle handle,
                                           std::uint64_t submittedTimeline) {
    if (!validType(type)) {
        return Halcyon::Result<void>::failure(invalidTypeError());
...
Halcyon::Result<void> BindlessTable::setDefault(DescriptorType type,
                                                DescriptorValue value) {
    if (!validType(type)) {
        return Halcyon::Result<void>::failure(invalidTypeError());
    }
...
std::optional<DescriptorValue> BindlessTable::get(DescriptorType type,
...
Halcyon::Result<DescriptorValue> BindlessTable::getResult(DescriptorType type,
    ...
    return Halcyon::Result<DescriptorValue>::failure(
        makeError(Halcyon::Core::ErrorCode::NotFound,
...
Halcyon::Core::Error BindlessTable::invalidTypeError() const {
    return makeError(Halcyon::Core::ErrorCode::InvalidArgument,
