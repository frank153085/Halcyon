...
{
    return Halcyon::Error{Halcyon::ErrorCode::InvalidArgument,
                          std::move(message), "camera"};
}

} // namespace

Halcyon::Result<glm::mat4> makeReversedZProjection(
    const Perspective& perspective)
{
    constexpr float pi = glm::pi<float>();
    if (!finite(perspective.verticalFovRadians) ||
        perspective.verticalFovRadians <= 0.0f ||
    ...
    const bool infiniteFar = std::isinf(perspective.farPlane) &&
                             perspective.farPlane > 0.0f;
    if (!infiniteFar &&
        (!finite(perspective.farPlane) ||
         perspective.farPlane <= perspective.nearPlane))
    {
        return Halcyon::Result<glm::mat4>::failure(
    ...
    const float focalLength =
        1.0f / std::tan(perspective.verticalFovRadians * 0.5f);
    ...
    else
    {
        const float inverseRange =
            1.0f / (perspective.farPlane - perspective.nearPlane);
        projection[2][2] = perspective.nearPlane * inverseRange;
        projection[3][2] = perspective.nearPlane * perspective.farPlane *
                           inverseRange;
    }
...
Halcyon::Result<void> Camera::setViewport(ViewportExtent extent)
{
    ...
    candidate.aspectRatio = static_cast<float>(extent.width) /
...
Halcyon::Result<void> Camera::setPosition(const glm::vec3& position)
{
    ...
    {
        return Halcyon::Result<void>::failure(
            invalidCameraArgument("position must be finite"));
    }
    position_ = position;
    return Halcyon::Result<void>::success();
}
...
Halcyon::Result<void> Camera::setOrientation(const glm::quat& cameraToWorld)
{
    ...
    if (!finite(cameraToWorld) || !finite(squaredLength) ||
        squaredLength <= 1.0e-12f)
    {
        return Halcyon::Result<void>::failure(
...
Halcyon::Result<void> Camera::lookAt(const glm::vec3& position,
                                     ...
                                     const glm::vec3& upHint)
{
    ...
    const glm::vec3 normalizedUp =
        glm::normalize(glm::cross(normalizedRight, normalizedForward));

    // Columns of a camera-to-world rotation are local +X, +Y and +Z.
    const glm::mat3 cameraToWorld{normalizedRight, normalizedUp,
                                  -normalizedForward};
...
{
    const glm::mat4 worldToCameraRotation =
        glm::mat4_cast(glm::conjugate(orientation_));
...
CameraData Camera::data() const noexcept
{
    ...
    const float farOrZero = std::isinf(perspective_.farPlane)
                                ? 0.0f
                                : perspective_.farPlane;
