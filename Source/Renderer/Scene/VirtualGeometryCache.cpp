    ...
    const VirtualGeometryCacheOptions* expectedOptions)
{
    ...
    try { file.resize(static_cast<std::size_t>(size)); }
    catch (const std::bad_alloc&) { return cacheAllocationError(); }
    stream.seekg(0); stream.read(reinterpret_cast<char*>(file.data()),
    ...
    Reader header{file}; header.offset = 8; std::uint32_t version = 0, endian = 0, maxVertices = 0, maxTriangles = 0, level = 0, meshoptVersion = 0; float simplifyError = 0.0f; std::array<float, 3> lodRatios{}; std::uint64_t payloadOffset = 0, rawSize = 0, compressedSize = 0;
    Sha256Digest source{};
    ...
    if (version != kVirtualGeometryCacheVersion || endian != kEndian ||
        meshoptVersion != MESHOPTIMIZER_VERSION || invalidRatio ||
        ...
        compressedSize > file.size() - header.offset ||
        compressedSize != file.size() - header.offset)
    ...
    const auto result = ZSTD_decompress(payload.data(), payload.size(),
        file.data() + header.offset, static_cast<std::size_t>(compressedSize));
    ...
    if (vertexCount > 100000000u || meshletVertexCount > 100000000u || triangleByteCount > 300000000u || indexCount > 300000000u || meshletCount > kMaxVisibilityMeshlets || lodCount > 1000000u || primitiveCount > 1000000u) return cacheError("cache counts are unreasonable");
    std::uint64_t expectedPayloadSize = 7u * sizeof(std::uint32_t);
    ...
    if (!addTableSize(vertexCount, 48u) ||
        ...
        !addTableSize(primitiveCount, 36u) || expectedPayloadSize != payload.size())
    ...
    if (metadata) { metadata->build.maxVertices = maxVertices; metadata->build.maxTriangles = maxTriangles; metadata->build.simplifyError = simplifyError; metadata->build.lodRatios = lodRatios; metadata->compressionLevel = level; }
