...
FrameBudgetController::FrameBudgetController() : FrameBudgetController(FrameBudgetConfig{}) {}

FrameBudgetController::FrameBudgetController(FrameBudgetConfig config) : config_(std::move(config)) {
    normalizeConfig();
...
void FrameBudgetController::normalizeQuality() noexcept {
    ...
    quality_.shadowResolutionLevel = static_cast<std::uint8_t>(std::min<std::size_t>(
        quality_.shadowResolutionLevel, kShadowResolutionScales.size() - 1u));
    quality_.shadowUpdateLevel = static_cast<std::uint8_t>(std::min<std::size_t>(
        quality_.shadowUpdateLevel, kShadowUpdatePeriods.size() - 1u));
    quality_.geometryLodLevel = static_cast<std::uint8_t>(std::min<std::size_t>(
        quality_.geometryLodLevel, kGeometryLodBiases.size() - 1u));
    quality_.rayTracingLevel = static_cast<std::uint8_t>(std::min<std::size_t>(
        quality_.rayTracingLevel, kRayTracingModes.size() - 1u));
}

void FrameBudgetController::reset() { reset(config_.initialQuality); }
...
void FrameBudgetController::setConfig(FrameBudgetConfig config) {
    ...
    if (decisionLog_.size() > config_.maxDecisionLogEntries) {
        const auto first = decisionLog_.end() -
                           static_cast<std::ptrdiff_t>(config_.maxDecisionLogEntries);
...
void FrameBudgetController::recordActualMeasurement(std::uint64_t frameIndex,
                                                     double gpuFrameMs) {
...
FrameBudgetUpdate FrameBudgetController::updateInternal(std::uint64_t frameIndex,
                                                        double gpuFrameMs) {
    ...
    if (gpuFrameMs > gpuBudgetMs_) {
        overBudgetStreak_ = std::min<std::uint32_t>(overBudgetStreak_ + 1u,
                                                    config_.downgradeAfterFrames);
        underBudgetStreak_ = 0;
    } else if (gpuFrameMs < gpuBudgetMs_ * config_.upgradeHeadroomFraction) {
        underBudgetStreak_ = std::min<std::uint32_t>(underBudgetStreak_ + 1u,
                                                      config_.upgradeAfterFrames);
        overBudgetStreak_ = 0;
    } else {
        overBudgetStreak_ = 0;
        underBudgetStreak_ = 0;
    ...
    decision.expectedGpuSavingsMs = direction == AdjustmentDirection::Downgrade ? selected.savings : 0.0;
    decision.expectedGpuCostMs = direction == AdjustmentDirection::Upgrade ? selected.cost : 0.0;
    decision.qualityLoss = direction == AdjustmentDirection::Downgrade ? selected.qualityDelta : 0.0;
    ...
    if (direction == AdjustmentDirection::Downgrade) {
        reason << toString(direction) << " " << toString(selected.knob) << ": GPU frame time "
               << gpuFrameMs << " ms exceeded " << gpuBudgetMs_
               << " ms for " << config_.downgradeAfterFrames << " consecutive frames";
    } else {
        reason << toString(direction) << " " << toString(selected.knob) << ": GPU frame time "
...
std::optional<FrameBudgetController::Candidate>
FrameBudgetController::chooseCandidate(AdjustmentDirection direction) const {
    ...
    for (std::uint8_t raw = 0; raw < static_cast<std::uint8_t>(QualityKnob::Count); ++raw) {
        ...
        if (direction == AdjustmentDirection::Downgrade) {
            ...
            candidate.score = candidate.savings /
                              std::max(candidate.qualityDelta, std::numeric_limits<double>::epsilon());
        } else {
...
std::uint8_t FrameBudgetController::level(QualityKnob knob) const noexcept {
    switch (knob) {
    case QualityKnob::InternalResolution: return quality_.internalResolutionLevel;
    case QualityKnob::ShadowResolution: return quality_.shadowResolutionLevel;
    case QualityKnob::ShadowUpdatePeriod: return quality_.shadowUpdateLevel;
    case QualityKnob::GeometryLod: return quality_.geometryLodLevel;
    case QualityKnob::RayTracing: return quality_.rayTracingLevel;
...
void FrameBudgetController::setLevel(QualityKnob knob, std::uint8_t value) noexcept {
    ...
    switch (knob) {
    case QualityKnob::InternalResolution: quality_.internalResolutionLevel = value; break;
    case QualityKnob::ShadowResolution: quality_.shadowResolutionLevel = value; break;
    case QualityKnob::ShadowUpdatePeriod: quality_.shadowUpdateLevel = value; break;
    case QualityKnob::GeometryLod: quality_.geometryLodLevel = value; break;
    case QualityKnob::RayTracing: quality_.rayTracingLevel = value; break;
    case QualityKnob::Count: break;
    }
}
...
double FrameBudgetController::gpuCost(QualityKnob knob, std::uint8_t value) const noexcept {
    ...
    switch (knob) {
    case QualityKnob::InternalResolution:
        return model.internalResolutionGpuMs[std::min<std::size_t>(value,
                                                                   model.internalResolutionGpuMs.size() - 1u)];
    case QualityKnob::ShadowResolution:
        return model.shadowResolutionGpuMs[std::min<std::size_t>(value,
                                                                 model.shadowResolutionGpuMs.size() - 1u)];
    case QualityKnob::ShadowUpdatePeriod:
        return model.shadowUpdateGpuMs[std::min<std::size_t>(value,
                                                             model.shadowUpdateGpuMs.size() - 1u)];
    case QualityKnob::GeometryLod:
        return model.geometryLodGpuMs[std::min<std::size_t>(value,
                                                            model.geometryLodGpuMs.size() - 1u)];
    case QualityKnob::RayTracing:
        return model.rayTracingGpuMs[std::min<std::size_t>(value,
                                                           model.rayTracingGpuMs.size() - 1u)];
    case QualityKnob::Count: break;
    }
    return 0.0;
}
...
double FrameBudgetController::qualityScore(QualityKnob knob, std::uint8_t value) const noexcept {
    ...
    switch (knob) {
    case QualityKnob::InternalResolution:
        return model.internalResolutionQuality[std::min<std::size_t>(value,
                                                                     model.internalResolutionQuality.size() - 1u)];
    case QualityKnob::ShadowResolution:
        return model.shadowResolutionQuality[std::min<std::size_t>(value,
                                                                   model.shadowResolutionQuality.size() - 1u)];
    case QualityKnob::ShadowUpdatePeriod:
        return model.shadowUpdateQuality[std::min<std::size_t>(value,
                                                               model.shadowUpdateQuality.size() - 1u)];
    case QualityKnob::GeometryLod:
        return model.geometryLodQuality[std::min<std::size_t>(value,
                                                              model.geometryLodQuality.size() - 1u)];
    case QualityKnob::RayTracing:
        return model.rayTracingQuality[std::min<std::size_t>(value,
                                                             model.rayTracingQuality.size() - 1u)];
    case QualityKnob::Count: break;
    }
    return 0.0;
}
