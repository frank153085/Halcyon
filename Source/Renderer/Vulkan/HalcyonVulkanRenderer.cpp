...
[[nodiscard]] std::string vkFailure(const char* operation, VkResult result)
{
    ...
    stream << operation << " failed (" << resultName(result) << ", " << static_cast<int>(result)
           << ')';
    return stream.str();
}
...
void destroyDebugMessenger(VkInstance instance, VkDebugUtilsMessengerEXT& messenger) noexcept
{
...
struct Renderer::Impl
{
    ...
    [[nodiscard]] VoidResult createDevice()
    {
        ...
        const bool canUseRayQuery = caps.rayQuery && config.rayQuery != FeatureMode::Disabled;
        if (canUseRayQuery)
        {
    ...
    [[nodiscard]] VoidResult createFrameResources()
    {
        const std::uint32_t frameCount =
            std::clamp(config.framesInFlight == 0 ? kDefaultFramesInFlight : config.framesInFlight,
                2u,
                kMaxFramesInFlight);
    ...
    [[nodiscard]] VoidResult createSwapchain()
    {
        ...
        else
        {
            ...
            const std::uint32_t requestedHeight =
                framebufferHeight > 0 ? static_cast<std::uint32_t>(framebufferHeight)
        ...
        result =
            vkGetSwapchainImagesKHR(device, newSwapchain, &imageCountReturned, newImages.data());
        if (result != VK_SUCCESS && result != VK_INCOMPLETE)
        {
            vkDestroySwapchainKHR(device, newSwapchain, nullptr);
    ...
    [[nodiscard]] FrameStats render(const FramePacket& packet)
    {
        ...
        signalInfos[0].semaphore = presentReadySemaphores[stats.swapchainImageIndex];
        ...
        presentInfo.pWaitSemaphores = &presentReadySemaphores[stats.swapchainImageIndex];
