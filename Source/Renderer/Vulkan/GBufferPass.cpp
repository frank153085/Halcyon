...
void addGBufferPass(Graph::FrameGraph& graph, FramePassContext& ctx)
{
    ...
    auto& gbufferPass = graph.addPass<Graph::FrameGraph::Empty>("G-buffer",
        ...
        [passCtx, hizPhase1Input](const Graph::FrameGraphResources& resources,
            const Graph::FrameGraph::Empty&, Graph::CommandContext&)
        {
            ...
            bool gpuDoubleSidedReady = false;
            VkDescriptorSet gpuDoubleSidedGraphicsSet = VK_NULL_HANDLE;
            if (gpuDrivenBindless && gpuIndirectCompatible &&
                pipelines.gpuDrivenGbufferDoubleSidedPipeline.pipeline() != VK_NULL_HANDLE &&
                pipelines.frustumCullPipeline.computePipeline() != VK_NULL_HANDLE)
            {
                if (ctx.gpuCsmCullSet == VK_NULL_HANDLE)
                {
                    ...
                    else
                    {
                        ...
                        writeStorageBuffer(ctx.gpuCsmCullSet, 4,
                            gpuSceneBuffers.shadowFrustum().occludedIndicesBuffer(), sceneBytes);
                    }
                }
                ...
                VkDescriptorSetAllocateInfo graphicsAlloc{VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO};
                graphicsAlloc.descriptorPool = ctx.descriptorPool;
                graphicsAlloc.descriptorSetCount = 1;
                graphicsAlloc.pSetLayouts = &pipelines.gpuSceneGraphicsLayout;
                if (ctx.gpuCsmCullSet != VK_NULL_HANDLE && ctx.gpuCsmIndirectSet != VK_NULL_HANDLE &&
                    vkAllocateDescriptorSets(device, &graphicsAlloc, &gpuDoubleSidedGraphicsSet) == VK_SUCCESS)
                {
                    const VkDeviceSize sceneBytes = std::max<VkDeviceSize>(4,
                        static_cast<VkDeviceSize>(gpuSceneBuffers.capacity()) * sizeof(std::uint32_t));
                    writeStorageBuffer(gpuDoubleSidedGraphicsSet, 0, gpuSceneBuffers.transformBuffer(),
                        static_cast<VkDeviceSize>(gpuSceneBuffers.capacity()) *
                            sizeof(Halcyon::Renderer::Scene::TransformRow));
                    writeStorageBuffer(gpuDoubleSidedGraphicsSet, 1, gpuSceneBuffers.meshMaterialBuffer(),
                        static_cast<VkDeviceSize>(gpuSceneBuffers.capacity()) *
                            sizeof(Halcyon::Renderer::Scene::MeshMaterialRow));
                    ...
                    writeStorageBuffer(gpuDoubleSidedGraphicsSet, 2, visible, sceneBytes);
                    if (gpuMaterialCount != 0)
                        writeStorageBuffer(gpuDoubleSidedGraphicsSet, 3, gpuSceneBuffers.materialBuffer(),
                    ...
                    cullConstants.requiredFlags = static_cast<std::uint32_t>(
                        Halcyon::Renderer::Scene::Ecs::RenderableFlags::DoubleSided);
            ...
            VkRect2D scissor{{0, 0}, swapchainExtent};
                 ...
                 gpuGraphicsSet != VK_NULL_HANDLE)
             {
                 ...
                 if (sets[0] != VK_NULL_HANDLE)
                 {
                     ...
                     if (gpuDoubleSidedReady)
                     {
                         vkCmdBindPipeline(frame.commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                             pipelines.gpuDrivenGbufferDoubleSidedPipeline.pipeline());
                         VkDescriptorSet doubleSets[2] = {sets[0], gpuDoubleSidedGraphicsSet};
                         vkCmdBindDescriptorSets(frame.commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                             pipelines.gpuDrivenGbufferDoubleSidedPipeline.layout(), 0, 2, doubleSets, 0, nullptr);
                         vkCmdPushConstants(frame.commandBuffer,
                             pipelines.gpuDrivenGbufferDoubleSidedPipeline.layout(),
                             VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(constants), &constants);
                         vkCmdBindVertexBuffers(frame.commandBuffer, 0, 1, &gpuVertexBuffer, &vertexOffset);
